...
本文档涵盖了 `1.x` 系列版本中的更新过程。这意味着从例如 `1.5.0` 更新到 `1.6.0`。**请勿使用本指南从 `0.7` 开始升级，这会导致你升级失败。**
...
如果您从 翼龙 _之前的_ `1.3.x` 版本升级，则需要更新 PHP 依赖项。最新版本的翼龙面板**最低支持 PHP 7.4 的版本** 和 Composer v2。
:::
...
**在继续之前**，请确保您的系统和网络服务器配置已通过运行 `php -v` 和 `composer --version` 来确认您的服务器已在 PHP 7.4 和 Composer 2 或更高的版本。您应该会看到类似于以下结果的输出。如果您没有看到 PHP 7.4 和 Composer 2 或更高的版本，您将需要按照我们的 [PHP 升级指南](/guides/php_upgrade.md) 进行升级，然后再返回此文档。
...
如果您不想执行自动升级，或者想要参考任何升级步骤，您可以按照下面的文档进行操作。
...
_每次_更新后，您都应该重新启动工作队列程序，以确保新的代码已加载并使用。
...
现在所有内容都已更新，您需要退出维护模式，以便面板可以继续运行并给用户服务。
